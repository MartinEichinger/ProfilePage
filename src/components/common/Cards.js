...
class Cards extends Component {
    ...
    render() {
        var {id, ref, type, cat, title, summary, finalDate, shortDesc, backend, frontend} = this.props.content;

        return ( 
            <div css={this.styleFlipCard} className="flip-card" id={id} >
                <div className="flip-card-inner" css={this.styleFlipCardInner} >
                    <div className="flip-card-front" css={this.styleFlipCardFront} >
                        <div className="img">
                            <div className="icon">
                                <a href={ref} target='_blank' rel='noreferrer'><PlayCircleFilledIcon /></a>
                            </div>
                        </div>
                        <h4>{type} - {cat}</h4>
                        <h1>{title}</h1>
                        <p>{summary}</p>
                        <div className="iconleft" onClick={() => this.onClickRotate({id})}>
                            <CachedIcon />
                        </div>
                    ...
                    <div className="flip-card-back" css={this.styleFlipCardBack} >
                        <h1>Short description:</h1>
                        <p>{shortDesc}</p>
                        <h1>Used technologies:</h1>
                        <p>Backend: {backend}</p>
                        <p>Frontend: {frontend}</p>
                        <div className="iconleft" onClick={() => this.onClickRotateOut({id})}>
                            <CachedIcon />
                        </div>
                    </div>
                </div>
